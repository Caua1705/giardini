...
new_content = """  <!-- ══ SECTION 5: LOCALIZAÇÃO (ULTIMATE EDITORIAL FIXED) ════════════════════════════ -->
  <style>
    /* ── ANIMATIONS ─────────── */
  ...
  <section id="location" class="py-32 md:py-48 w-full relative z-[60] bg-[var(--c-bg)] overflow-hidden" style="border-top: 1px solid rgba(26,26,26,0.04);">
    
    <!-- Atmospheric Orbs & Watermark (Removed mix-blend-modes to fix black/white flashing bug) -->
    <div class="ed-watermark">Giardini</div>
    <div class="absolute top-[20%] left-[-10%] w-[800px] h-[800px] bg-[#EAE4D9] rounded-full blur-[120px] opacity-30 pointer-events-none transition-transform duration-[20s] hover:translate-x-12"></div>
    <div class="absolute bottom-[20%] right-[10%] w-[600px] h-[600px] bg-[var(--c-primary)] rounded-full blur-[150px] opacity-[0.03] pointer-events-none transition-transform duration-[25s] hover:-translate-x-12"></div>
    ...
    <div class="max-w-[1340px] px-8 md:px-12 mx-auto grid grid-cols-1 lg:grid-cols-12 gap-24 lg:gap-16 items-start relative z-10 w-full">
      ...
      <div class="lg:col-span-5 flex flex-col pt-4">
        ...
        <!-- Label Alinhada idêntica às outras seções -->
        <div class="anim-fade-blur flex items-center justify-between mb-10 opacity-60">
          <div class="text-[10px] md:text-xs text-[var(--c-dark)] font-bold uppercase tracking-[0.2em] flex items-center gap-3">
            <span class="w-6 h-px bg-[var(--c-dark)] block opacity-30"></span>
            Localização
          </div>
        </div>
        ...
        <h2 style="font-family:'Instrument Serif',serif; letter-spacing:-0.04em;" class="text-[5rem] md:text-[7.5rem] leading-[0.82] text-[var(--c-dark)] mb-10 overflow-hidden pb-4">
          <span class="block overflow-hidden"><span class="block ed-reveal anim-fade-blur" style="animation-delay: 0.1s">Visite</span></span>
          <span class="block overflow-hidden"><span class="block ed-reveal anim-fade-blur" style="animation-delay: 0.2s">Nossa <span class="italic text-[var(--c-primary)] font-light pr-4">Casa</span></span></span>
        </h2>
        
        <!-- Descrição Editorial -->
        <div class="overflow-hidden mb-24">
          <p class="text-[var(--c-dark)]/65 font-light text-xl md:text-2xl pt-2 leading-[1.6] max-w-[420px] ed-reveal anim-fade-blur" style="animation-delay: 0.3s">
        ...
        <div class="space-y-16 mb-20 relative">
          ...
          <div class="ed-data-row pl-10 pr-2 pb-2 group">
            ...
            <div class="ed-reveal anim-fade-blur" style="animation-delay: 0.4s">
          ...
          <div class="ed-data-row pl-10 pr-2 group">
            <div class="ed-fine-line"></div>
            <div class="ed-reveal anim-fade-blur flex flex-col gap-8" style="animation-delay: 0.5s">
        ...
        <div class="ed-reveal anim-fade-blur" style="animation-delay: 0.6s">
          <a href="https://maps.google.com" target="_blank"
            ...
            <div class="relative z-10 flex w-full h-full items-center justify-between rounded-full bg-[var(--c-dark)] px-8 py-5 border border-white/5 shadow-2xl">
              <!-- Icon brightness adjusted so it is visible against dark button -->
              <span class="text-[10px] md:text-xs font-bold uppercase tracking-[0.25em] text-white flex items-center gap-3">
                <iconify-icon class="text-lg text-[var(--c-bg)] opacity-80 group-hover:text-[var(--c-primary)] transition-colors" icon="solar:map-point-wave-bold-duotone"></iconify-icon>
                Como Chegar
              </span>
      ...
      <div class="lg:col-span-7 w-full h-full flex items-center mt-20 lg:mt-0 relative" style="perspective: 2000px;">
        ...
        <div class="absolute inset-y-8 left-12 right-[-20%] bg-[#EAE4D9]/40 z-0 hidden lg:block ed-reveal anim-fade-blur" style="animation-delay: 0.3s"></div>
        
        <div class="ed-map-container relative z-10 w-[105%] md:w-full ml-[-2.5%] md:ml-0 aspect-[4/5] md:aspect-[5/6] overflow-hidden p-[1px] cursor-pointer bg-[var(--c-dark)]/5 ed-reveal anim-fade-blur group" style="animation-delay: 0.4s" onclick="window.open('https://maps.google.com', '_blank')">
          <div class="w-full h-full overflow-hidden relative bg-[#EAE4D9]">
            
            <!-- Cinematic Color Grade Overlays (Removed multiply/color blend modes to fix browser inconsistencies, just pure opacity) -->
            <div class="absolute inset-0 z-10 bg-[var(--c-bg)] opacity-60 pointer-events-none transition-opacity duration-1000 group-hover:opacity-40"></div>
            <div class="absolute inset-0 z-10 bg-[var(--c-primary)] opacity-10 pointer-events-none transition-opacity duration-1000 group-hover:opacity-0"></div>
            ...
            <div class="absolute bottom-8 left-8 right-8 flex justify-between items-end z-20 pointer-events-none px-2 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-700">
              <span class="text-[9px] font-bold uppercase tracking-widest drop-shadow-lg">Navegação via maps</span>
              <span class="text-[10px] uppercase font-mono tracking-widest bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/20">Giardini</span>
            </div>
...
# Using regex to match the ULTIMATE EDITORIAL block that I injected earlier.
pattern = re.compile(r'<!--\s*══ SECTION 4: LOCALIZAÇÃO \(ULTIMATE EDITORIAL\)[\s\S]*?</section>', re.IGNORECASE)

match = pattern.search(html)
if match:
    print("Found ULTIMATE EDITORIAL block! Replacing with FIXED version successfully.")
    html = pattern.sub(new_content, html)
else:
    print("Could not find ULTIMATE EDITORIAL block. Attempting fallback match...")
    fallback_pattern = re.compile(r'<!--\s*══ SECTION 4: LOCALIZAÇÃO \(GEMINI 3 AESTHETIC\)[\s\S]*?</section>', re.IGNORECASE)
    match_fallback = fallback_pattern.search(html)
    if match_fallback:
        print("Found GEMINI 3 block! Replacing successfully.")
        html = fallback_pattern.sub(new_content, html)
    else:
        print("Failed to match. Please inspect HTML.")

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(html)
print("Updated hero_premium.html with ULTIMATE EDITORIAL FIXED UI")
